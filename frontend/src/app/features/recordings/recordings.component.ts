    ...
    <app-page-layout 
      ...
      (sidebarToggle)="onSidebarToggle($event)">
          ...
          <div *ngIf="isLoading" class="flex flex-col items-center justify-center py-20">
            <div class="relative">
              <!-- Video icon with spinning effect -->
              <div class="w-16 h-16 bg-green-100 rounded-lg flex items-center justify-center">
                <svg class="w-8 h-8 text-green-600 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                </svg>
              </div>
              <!-- Spinning ring around icon -->
              <div class="absolute -inset-2 w-20 h-20 border-2 border-green-200 border-t-green-600 rounded-full animate-spin"></div>
            </div>
            <div class="mt-6 text-center">
              <h3 class="text-lg font-medium text-gray-900 mb-2">Loading Recordings</h3>
              <p class="text-sm text-gray-500">Fetching your lecture recordings...</p>
              <!-- Skeleton cards -->
              ...
              <div class="p-6">
                ...
                <div class="flex items-center justify-between">
                  <div class="text-sm text-gray-500">
                    <span class="font-medium">{{ recording.uploaded_by_name }}</span>
                    <span class="mx-2">•</span>
                    <span>{{ recording.created_at | date:'MMM d, y' }}</span>
                  </div>
...
export class RecordingsComponent implements OnInit {
  ...
  filterRecordings(): void {
    if (this.selectedSubject && this.selectedSubject !== 'All Courses') {
      this.filteredRecordings = this.recordings.filter(r => r.title.toLowerCase().includes(this.selectedSubject.toLowerCase()));
    } else {
